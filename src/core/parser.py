...
class I18nFileParser:
    ...
    def parse_single_file(self, file_path: str) -> Optional[I18nFileInfo]:
        ...
        try:
            ...
            if result.error and ('不存在' in result.error or 'unsupported' in result.error or '无法获取解析器' in result.error):
                return None
            return result
        except Exception as e:
    ...
    def get_key_value(self, key: str, file_path: str = None) -> Optional[Any]:
        ...
        if file_path:
            target_files = [f for f in self.parsed_files 
                          if f.file_path == file_path or f.relative_path == file_path]
    ...
    def _find_duplicate_keys(self) -> Dict[str, List[str]]:
        ...
        # 只返回出现在多个文件中的键
        duplicate_keys = {
            key: files for key, files in key_files.items()
            if len(files) > 1
        }
        ...
        for lang, files in language_groups.items():
            ...
            for key in all_keys:
                ...
                if files_without_key:  # 存在不一致
                    ...
                    inconsistent_keys[key][lang] = {
                        'has_key': files_with_key,
                        'missing_key': files_without_key
                    }
