...
class StatsWidget(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.stats_cards = {}
        stats_items = [
            ("total_keys", "总键数", "#2196F3"),
            ("missing_keys", "缺失键", "#F44336"),
            ("unused_keys", "未使用键", "#FF9800"),
    ...
    def create_coverage_widget(self) -> QWidget:
        ...
        def paint_event(event):
            ...
            if self.coverage_value > 0:
                ...
                span_angle = -(self.coverage_value / 100) * 360 * 16
    ...
    def update_stats(self, analysis_result) -> None:
        """更新统计信息"""
        # 更新覆盖率
        self.coverage_value = analysis_result.coverage_percentage
        self.coverage_widget.update()
        
        # 更新统计卡片
        self.stats_cards["total_keys"].value_label.setText(str(analysis_result.total_defined_keys))
        self.stats_cards["missing_keys"].value_label.setText(str(len(analysis_result.missing_keys)))
        self.stats_cards["unused_keys"].value_label.setText(str(len(analysis_result.unused_keys)))
        self.stats_cards["inconsistent_keys"].value_label.setText(str(len(analysis_result.inconsistent_keys)))
...
class ResultWidget(QWidget):
    ...
    def update_inconsistent_keys_table(self, inconsistent_keys: List) -> None:
        ...
        for i, inconsistent_key in enumerate(inconsistent_keys):
            self.inconsistent_table.setItem(i, 0, QTableWidgetItem(inconsistent_key.key))
            self.inconsistent_table.setItem(i, 1, QTableWidgetItem(", ".join(inconsistent_key.existing_files)))
            self.inconsistent_table.setItem(i, 2, QTableWidgetItem(", ".join(inconsistent_key.missing_files)))
