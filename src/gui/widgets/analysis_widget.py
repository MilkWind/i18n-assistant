...
class AnalysisWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_message.emit("INFO", f"扫描完成: 找到 {scan_summary.total_files} 个文件，{scan_summary.total_matches} 个匹配项")
            ...
            self.log_message.emit("INFO", f"解析完成: 找到 {len(parse_result.files)} 个国际化文件，{len(parse_result.total_keys)} 个键")
