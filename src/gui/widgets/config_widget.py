...
class ConfigWidget(QWidget):
    ...
    def create_scan_group(self) -> QGroupBox:
        ...
        self.ignore_patterns_edit.setPlaceholderText("node_modules/**\n.git/**\ndist/**\nbuild/**")
    ...
    def browse_project_path(self) -> None:
        """浏览项目路径"""
        path = QFileDialog.getExistingDirectory(self, "选择项目目录", self.project_path_edit.text())
        if path:
            self.project_path_edit.setText(path)

    def browse_i18n_path(self) -> None:
        """浏览国际化目录"""
        path = QFileDialog.getExistingDirectory(self, "选择国际化目录", self.i18n_path_edit.text())
        if path:
            self.i18n_path_edit.setText(path)

    def browse_output_path(self) -> None:
        """浏览输出目录"""
        path = QFileDialog.getExistingDirectory(self, "选择输出目录", self.output_path_edit.text())
        if path:
    ...
    def get_config(self) -> Config:
        """从界面获取配置"""
        # 获取选中的文件扩展名
        selected_extensions = [ext for ext, cb in self.file_ext_checkboxes.items() if cb.isChecked()]

        # 获取忽略模式
        ignore_patterns = [line.strip() for line in self.ignore_patterns_edit.toPlainText().split('\n') if line.strip()]

        # 获取i18n模式
        i18n_patterns = [line.strip() for line in self.i18n_patterns_edit.toPlainText().split('\n') if line.strip()]

        # 创建配置对象
        config = Config(project_path=self.project_path_edit.text().strip(),
            i18n_path=self.i18n_path_edit.text().strip(), output_path=self.output_path_edit.text().strip(),
            file_extensions=selected_extensions, ignore_patterns=ignore_patterns,
            parser_type=self.parser_type_combo.currentText(), encoding=self.encoding_combo.currentText(),
            max_threads=self.max_threads_spin.value(), i18n_patterns=i18n_patterns)
    ...
    def reset_to_default(self) -> None:
        """重置到默认配置"""
        reply = QMessageBox.question(self, "确认重置", "确定要重置到默认配置吗？这将清除所有当前设置。",
