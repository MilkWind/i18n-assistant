...
class JsonI18nParser(BaseParser):
        ...
        try:
            ...
            if not content.strip():
                ...
                if validation_errors:
                    raise ParseError(f"JSON结构验证失败: {', '.join(validation_errors)}", file_path)

            # Create result object with flattened keys
            keys = self.flatten_keys(data)
            result = JsonParseResult(file_path=file_path, data=data, keys=keys, parser_type="json")

            logger.debug(f"成功解析JSON文件: {file_path}")
            return result

        except json.JSONDecodeError as e:
            raise ParseError(f"JSON解析错误: {e.msg}", file_path, e.lineno)
    ...
    def merge_data(self, base_data: Dict[str, Any], new_data: Dict[str, Any], overwrite: bool = False) -> Dict[
        str, Any]:
        """
        合并两个JSON数据
    ...
    def get_statistics(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stats = {'total_keys': len(keys), 'max_nesting_level': _count_nested_levels(data), 'empty_values': 0,
            'nested_objects': 0, 'array_values': 0}
    ...
    def find_duplicated_values(self, data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        # 只返回重复的值
        duplicated = {value: keys_list for value, keys_list in value_to_keys.items() if len(keys_list) > 1}
