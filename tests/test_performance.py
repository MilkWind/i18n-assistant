...
class TestPerformanceBenchmarks:
    ...
    def test_analysis_performance(self):
        ...
        file_info = I18nFileInfo(
            ...
            parser_type='json',
            file_size=1024,
            keys=set(all_keys.keys()),
            data=all_keys
        )
        
        parse_result = ParseResult(
            files=[file_info],
            total_keys=set(all_keys.keys()),
            duplicate_keys={},
            inconsistent_keys={},
            parse_errors=[]
        )
    ...
    def test_regex_performance(self):
        ...
        for i in range(1000):
            test_content.extend([
                f"const text{i} = t('test.key{i}');",
                f"const other{i} = 'not a translation';",
                f"function func{i}() {{ return $t('another.key{i}'); }}",
                f"// Comment line {i}",
                f"/* Block comment {i} */"
            ])
